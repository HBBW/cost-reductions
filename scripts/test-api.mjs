...
async function main() {
  ...
  // Replicate exact query from dashboard.routes.js with the same param binding
  const deptFilterSql = ` AND d.id IN (?)`;
  const deptFilterParams = ['0800'];
  ...
  req.input('p1', sql.Int, 2026);
  req.input('p2', sql.Int, 2026);
  req.input('p3', sql.Int, 2026);
