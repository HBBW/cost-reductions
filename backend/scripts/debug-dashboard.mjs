...
async function main() {
  ...
  // Test deptFilter logic
  const ph = deptIds.map(() => '?').join(',');
  const deptFilterSql = ` AND d.id IN (${ph})`;
  const deptFilterParams = [...deptIds];
  console.log('deptFilterSql:', deptFilterSql);
  console.log('deptFilterParams:', deptFilterParams);
  ...
  const sqlText = `
    ...
    WHERE d.is_active = 1${deptFilterSql}
    ORDER BY d.name`;
  
  const req = pool.request();
  req.input('p1', sql.Int, year);
  req.input('p2', sql.Int, year);
  req.input('p3', sql.Int, year);
  deptFilterParams.forEach((p, i) => req.input(`dept${i}`, sql.VarChar, p));
